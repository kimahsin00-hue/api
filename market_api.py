...
async def load_item_dump(force: bool = False) -> str:
    """
    force=False : items_v2.json 파일이 있으면 바로 로컬 파일 사용 (시간 무관) - 봇 시작 시 호출
    force=True  : arsha dump를 새로 받아서 ITEM_LIST와 items_v2.json 갱신 - /아이템디비갱신 명령어가 호출
    반환값: 결과 요약 문자열 (로그/응답용)
    """
    if not force and os.path.exists(ITEM_DB_FILE):
        with open(ITEM_DB_FILE, "r", encoding="utf-8") as f:
            backup = json.load(f)
        ITEM_LIST.update(backup)
        ITEM_LIST.update(HARDCODED_ITEMS)
        msg = f"↩️ 로컬 백업({ITEM_DB_FILE}) 사용: {len(backup)}개 로드됨. 현재 ITEM_LIST 총 {len(ITEM_LIST)}개"
        print(msg)
        return msg
