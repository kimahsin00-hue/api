...
async def load_item_dump(force: bool = False) -> str:
    """
    항상 arsha 라이브 dump를 먼저 시도합니다 (봇 시작 시 / force=True 관계없이 동일).
    dump 요청이 실패했을 때만 로컬 백업(items_v2.json)으로 폴백합니다.
    force 인자는 더 이상 분기 조건이 아니라, /아이템디비갱신 명령어 호출을 구분하는
    용도로만 남겨둡니다 (메시지 문구 등에 활용 가능).
