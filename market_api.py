...
_pa_token_cache = {"token": None, "cookie_jar": None}


async def _get_pa_token_session() -> aiohttp.ClientSession:
    """
    캐시된 토큰/세션이 있으면 재사용하고, 없으면 새로 발급받습니다.
    실패(만료 등) 시 호출부에서 _pa_token_cache를 초기화하고 재시도하세요.
    """
    if _pa_token_cache["token"] and _pa_token_cache["cookie_jar"]:
        session = aiohttp.ClientSession(
            headers={"User-Agent": "Mozilla/5.0"},
            cookie_jar=_pa_token_cache["cookie_jar"],
        )
        return session

    session = aiohttp.ClientSession(headers={"User-Agent": "Mozilla/5.0"})
    async with session.get("https://trade.kr.playblackdesert.com/Home/list/hot", timeout=aiohttp.ClientTimeout(total=10)) as resp:
        html = await resp.text()

    m = re.search(r'name="__RequestVerificationToken"[^>]*value="([^"]+)"', html)
    if not m:
        await session.close()
        raise RuntimeError("CSRF 토큰을 페이지에서 못 찾음 (페이지 구조가 바뀌었을 수 있음)")

    _pa_token_cache["token"] = m.group(1)
    _pa_token_cache["cookie_jar"] = session.cookie_jar
    return session


async def fetch_pa_search(keyword: str):
    """PA 공식 거래소 이름 검색. 토큰 만료 시 한 번 재발급 후 재시도."""
    for attempt in range(2):
        session = await _get_pa_token_session()
        try:
            async with session.post(
                "https://trade.kr.playblackdesert.com/Home/GetWorldMarketSearchList",
                data={"__RequestVerificationToken": _pa_token_cache["token"], "searchText": keyword},
                timeout=aiohttp.ClientTimeout(total=10),
            ) as resp:
                if resp.status != 200:
                    raise RuntimeError(f"검색 API status {resp.status}")
                data = await resp.json(content_type=None)
                if data.get("resultCode") != 0:
                    # 토큰 만료로 의심되는 경우 캐시 초기화 후 1회 재시도
                    _pa_token_cache["token"] = None
                    _pa_token_cache["cookie_jar"] = None
                    if attempt == 0:
                        continue
                    raise RuntimeError(f"검색 API 에러: {data.get('resultMsg')}")
                return [
                    {"id": it["mainKey"], "name": it["name"], "grade": it.get("grade")}
                    for it in data.get("marketList", data.get("list", []))
                ]
        finally:
            await session.close()
    return []
